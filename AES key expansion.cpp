/* Basic implementation of AES Key Expansion in CPP */


#include <iostream>
#include <iomanip>

using namespace std;

const uint8_t sbox[256] = {
    
};

const uint8_t Rcon[11] = {
    0x00, 0x01, 0x02,
    0x04, 0x08, 0x10,
    0x20, 0x40, 0x80,
    0x1b, 0x36
};
void xorBytes(const uint8_t* input1, const uint8_t* input2, uint8_t* output, size_t length) {
    for (size_t i = 0; i < length; ++i) {
        output[i] = input1[i] ^ input2[i];
    }
}

void rotWord(const uint8_t* word, uint8_t* result){
    result[0] = word[1];
    result[1] = word[2];
    result[2] = word[3];
    result[3] = word[0];
}
void subWord(const uint8_t* word, uint8_t* result) {
    for (size_t i = 0; i < 4; ++i) {
        result[i] = sbox[word[i]];
    }
}

void keyexpansion(const uint8_t key[], uint8_t w[][4], size_t rounds, size_t column){
    uint8_t temp[4];
    size_t n = 4;
    
    for (size_t i = 0; i < column; ++i) {
        w[i][0] = key[4 * i];
        w[i][1] = key[4 * i + 1];
        w[i][2] = key[4 * i + 2];
        w[i][3] = key[4 * i + 3];
    }
    
    for (size_t i = column; i < n * (rounds + 1); ++i) {
        temp[0] = w[i - 1][0];
        temp[1] = w[i - 1][1];
        temp[2] = w[i - 1][2];
        temp[3] = w[i - 1][3];

        if (i % column == 0) {
            uint8_t rcon[4] = { static_cast<uint8_t>(Rcon[i / column]), 0x00, 0x00, 0x00 };
            uint8_t temp2[4];
            rotWord(temp, temp2); //lcs-1 byte
            subWord(temp2, temp2); //substitute from s-box
            xorBytes(temp2, rcon, temp2, 4); //perform xor 
            temp[0] = temp2[0];
            temp[1] = temp2[1];
            temp[2] = temp2[2];
            temp[3] = temp2[3];
        }

        w[i][0] = w[i - column][0] ^ temp[0];
        w[i][1] = w[i - column][1] ^ temp[1];
        w[i][2] = w[i - column][2] ^ temp[2];
        w[i][3] = w[i - column][3] ^ temp[3];
    }
}
void print(const uint8_t w[][4], size_t numWords) {
    std::cout << "\n\nKeywords: \n";
    for (size_t i = 0; i < numWords; ++i) {
        std::cout << "w" << i << " = ";
        for (size_t j = 0; j < 4; ++j) {
            std::cout << std::hex << std::setw(2) << std::setfill('0') << static_cast<int>(w[i][j]) << " ";
        }
        std::cout << std::dec << std::endl;
    }
}

int main() {
    
    const uint8_t key[16] = {
        0x0f, 0x15, 0x71, 0xc9,
        0x47, 0xd9, 0xe8, 0x59,
        0x0c, 0xb7, 0xad, 0xd6,
        0xaf, 0x7f, 0x67, 0x98,
    };
    
    const size_t rounds = 10;
    const size_t column = 4;
    
    uint8_t w[44][4] = {0};
    
    keyexpansion(key,w,rounds,column);
    
    print(w, 44);
    
    return 0;
}


